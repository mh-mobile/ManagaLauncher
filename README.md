...
- **バックログ管理** - 連載状態（連載中/休載/完結）× 読書状態（追っている/積読/読了）の2軸で作品を整理
- **ライブラリ・検索** - 全作品の一覧表示と2軸フィルター検索
- **キャッチアップ** - Tinder風カードスワイプで未読マンガを一括チェック
- **アクティビティタイムライン** - Swift Chartsによる週別読書グラフで記録を振り返り
- **テーマ** - Classic / Kinetic Ink / レトロの3テーマから選択
- **壁紙** - プリセットカラー / カスタムカラー / 写真から好みの背景を設定
- **更新スケジュール** - 毎週 / 隔週 / 月1回 / カスタム間隔で更新頻度を管理、次回更新日バッジ表示
- **グリッド表示** - Pinterest風Masonryレイアウト、画面幅に応じて2〜4列の可変レスポンシブ
- **ウィジェット** - ホーム画面・ロック画面・コントロールセンターに対応
...
- Swift / SwiftUI
- SwiftData + CloudKit
- WidgetKit（ホーム / ロック画面 / コントロールセンター）
- AppIntents
- Swift Charts
