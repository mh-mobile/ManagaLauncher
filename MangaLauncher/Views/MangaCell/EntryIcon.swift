    ...
    var body: some View {
        if let imageData = entry.imageData, let image = imageData.toSwiftUIImage() {
            image
                ...
                .clipShape(RoundedRectangle(cornerRadius: size > 40 ? InkTheme.cardCornerRadius : InkTheme.cornerRadius))
                .accessibilityLabel("\(entry.name)のアイコン")
        } else {
            RoundedRectangle(cornerRadius: size > 40 ? InkTheme.cardCornerRadius : InkTheme.cornerRadius)
